...
class KeldocCenter:
    ...
    def fetch_vaccine_cabinets(self):
        ...
        for specialty in self.vaccine_specialties:
            ...
            try:
                ...
                continue
            except httpx.ConnectError as hex:
                logger.warning(
                    f"Keldoc request connect error for center: {self.base_url} (vaccine cabinets)")
                return None
    ...
    def fetch_center_data(self):
        ...
        try:
            ...
            logger.warning(
                f"Keldoc request connect error for center: {self.base_url} (center info)")
            return None
        except httpx.HTTPStatusError as hex:
            logger.warning(
                f"Keldoc request returned error {hex.response.status_code} "
    ...
    def parse_resource(self):
        ...
        try:
            ...
            logger.warning(
                f"Keldoc request connect error for center: {self.base_url} (resource)")
            return None
        except httpx.HTTPStatusError as hex:
            logger.warning(
    ...
    def get_timetables(self, start_date, motive_id, agenda_ids):
        ...
        end_date = (isoparse(start_date) + timedelta(days=KELDOC_PAGE_SIZE)).strftime("%Y-%m-%d")
        logger.debug(
        ...
        try:
            ...
            logger.warning(
                f"Keldoc request connect error for center: {self.base_url} (calendar request)"
                f" celendar_url: {calendar_url}"
                f" calendar_params: {calendar_params}"
            )
            return None
    ...
    def count_appointements(self, appointments: list, start_date: datetime, end_date: datetime) -> int:
        paris_tz = timezone("Europe/Paris")
        count = 0
    ...
    def get_appointment_schedule(self, appointments: list, start_date: datetime, end_date: datetime, schedule_name: str) -> dict:
    ...
    def find_first_availability(self, start_date: str):
        ...
        for relevant_motive in self.vaccine_motives:
            ...
            s_date = isoparse(start_date)
            max_date = s_date + timedelta(days=KELDOC_SLOT_LIMIT)
            while s_date < max_date:
                timetables = self.get_timetables(s_date.strftime("%Y-%m-%d"), motive_id, agenda_ids)
                s_date += timedelta(days=KELDOC_PAGE_SIZE)
        ...
        s_datetime = (paris_tz.localize(isoparse(start_date) + timedelta(days=0)))
        n_datetime = s_datetime + timedelta(days=2, seconds=-1)
        appointment_schedules.append(
            self.get_appointment_schedule(appointments, s_datetime, n_datetime, "chronodose")
        )
        for n in INTERVAL_SPLIT_DAYS:
            n_datetime = s_datetime + timedelta(days=n, seconds=-1)
            appointment_schedules.append(
                self.get_appointment_schedule(appointments, s_datetime, n_datetime, f"{n}_days")
            )
